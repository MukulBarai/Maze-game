...
function updateFruit(){
  if(nextDir == dir.right) {
      if(!cells[fruit.xcor+1][fruit.ycor].fill){
          fruit.xcor++;
          curDir = nextDir;
          return;
      }
...
function update(){
  ...
  queue.push(cells[bird.xcor][bird.ycor]);
  visited[bird.xcor][bird.ycor] = true;
  path[bird.xcor][bird.ycor] = cells[bird.xcor][bird.ycor];
  ...
  while(queue.length){
    ...
    if(!cells[cell.xcor+1][cell.ycor].fill){
      if(!visited[cell.xcor+1][cell.ycor]){
        ...
        visited[cell.xcor+1][cell.ycor] = true;
      }
    }
    if(!cells[cell.xcor][cell.ycor+1].fill){
      if(!visited[cell.xcor][cell.ycor+1]){
        ...
        visited[cell.xcor][cell.ycor+1] = true;
      }
    }
    if(!cells[cell.xcor-1][cell.ycor].fill){
      if(!visited[cell.xcor-1][cell.ycor]){
  ...
  var nextCell = cells[fruit.xcor][fruit.ycor];
  while(path[nextCell.xcor][nextCell.ycor].xcor != bird.xcor || path[nextCell.xcor][nextCell.ycor].ycor != bird.ycor){
    nextCell = path[nextCell.xcor][nextCell.ycor];
  }

  bird.xcor = nextCell.xcor;
  bird.ycor = nextCell.ycor;
}
